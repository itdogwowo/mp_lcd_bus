...
def test_async_write():
    ...
    for i in range(0, half, 2):
        top_buf[i] = 0x00       # 綠
        top_buf[i + 1] = 0xE0
    tid = bus.write(top_buf, x=0, y=0, w=WIDTH, h=HEIGHT // 2)
    ...
    for i in range(0, half, 2):
        bot_buf[i] = 0x1F       # 藍
        bot_buf[i + 1] = 0x00
    tid = bus.write(bot_buf, x=0, y=HEIGHT // 2, w=WIDTH, h=HEIGHT // 2)
